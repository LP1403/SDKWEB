...
const vu = window.vu || {};
vu.sop = vu.sop || {};
vu.sop.screenTools = vu.sop.screenTools || {};
...
vu.sop.screenTools.initialize = function(camera) {
    moduleCamera = camera;
};
...
vu.sop.screenTools.isVideoVertical = function() {
    if (typeof moduleCamera.video.videoWidth !== 'undefined') {
        if (moduleCamera.video.videoWidth > moduleCamera.video.videoHeight) {
            return true
        } else {
            return false
        }
...
vu.sop.screenTools.goFullScreen = function() {
    ...
    invert = false;
    if (typeof moduleCamera.video !== 'undefined') {
        if (moduleCamera.video.videoHeight > moduleCamera.video.videoWidth) {
            invert = true;
            vu.sop.screenTools.width = moduleCamera.video.videoHeight;
            vu.sop.screenTools.height = moduleCamera.video.videoWidth;
        } else {
            vu.sop.screenTools.width = moduleCamera.video.videoWidth;
            vu.sop.screenTools.height = moduleCamera.video.videoHeight;
        }
    }
...
export default vu.sop.screenTools;
...
vu.sop.barcode = vu.sop.barcode || {};
vu.sop.barcode.ui = vu.sop.barcode.ui || {};
...
vu.sop.barcode.initialize = function(camera) {
...
vu.sop.barcode.documentId = null;
...
vu.sop.barcode.load = async function(basePath, techStack) {
    //await vu.sop.loadJs(basePath + '/js/libs/zxing-wasm/index.js');
    await vu.extras.loadScript(basePath, techStack, "libs/zxing-wasm", "index.js", "ZXingWASM");
...
vu.sop.barcode.loopRunning = true;

vu.sop.barcode.loop = async function() {
    if(!vu.sop.barcode.loopRunning)
        return;
    ...
    if (vu.sop.barcode.canvas === null) {
        ...
        vu.sop.barcode.canvas.width = moduleCamera.video.videoWidth;
        vu.sop.barcode.canvas.height = moduleCamera.video.videoHeight;
    }

    vu.sop.barcode.canvasContext.drawImage(moduleCamera.video, 0, 0,
    ...
    if (result.length > 0) {
        ...
        vu.sop.barcode.loopRunning = false;
        ...
        if (vu.sop.barcode.documentId === null){
            vu.sop.barcode.ui.resolve(resultByType);
            return;
        }

        let VUId = vu.sop.documentCodes.getVUIdFromId(vu.sop.barcode.documentId)
        let resultParsed = vu.sop.barcode.parse(resultByType, VUId)
...
vu.sop.barcode.ui.showTutorial = function() {
    let barcodeImage = vu.sop.barcode.ui.pdf417Image;
    // TODO -  Cambiar la imagen dependiendo del documento

    let text = vu.sop.msg.readBarcodeTutorial + "<br><img src='" + barcodeImage + "'><br><br>";

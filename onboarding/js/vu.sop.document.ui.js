...
vu.sop.document.ui.initialize = function(camera) {
...
vu.sop.document.ui.start = async function(side) {
    ...
    vu.sop.document.ui.canvasContext = vu.sop.document.ui.canvas.getContext("2d", { willReadFrequently: true });
...
vu.sop.document.ui.loop = async function(promise) {
    ...
    let vWidth = moduleCamera.video.videoWidth;
    let vHeight = moduleCamera.video.videoHeight;
    let result;
    let box;
    ...
    try {
        ...
        box = await vu.sop.document.objectDetection.predictAsync(moduleCamera.video)
    ...
    if (vu.sop.document.ui.results.length  >  3){
        ...
        for (let i = 0; i < vu.sop.document.ui.results.length; i++) {
            let time = vu.sop.document.ui.resultsTime[vu.sop.document.ui.results.length - i]
        ...
        // Photo
        let takePhoto = true
        for (let i = startPhotoIndex; i < vu.sop.document.ui.results.length; i++) {
        ...
        if (vu.sop.document.ui.checkPictureQuality) {
            if (box && typeof box[0] !== 'undefined') {
                ...
                let horizontalCenterOffset = Math.round((vu.sop.document.ui.height - box[0][1][2]) / 4)
                let verticalCenterOffset = Math.round((vu.sop.document.ui.width - box[0][1][3]) / 4)
                vu.sop.document.ui.canvasContext.drawImage(moduleCamera.video,
                ...
                let isBright = vu.image.brightnessDetector.isBrightAsync(vu.sop.document.ui.canvas)
                let isBlurryStartTime = new Date();
                let isBlurry = vu.image.blurDetector.isBlurryAsync(vu.sop.document.ui.canvas)
                let hasABrightSpotStartTime = new Date();
                let hasABrightSpot = vu.image.brigthSpotDetector.hasABrightSpot(vu.sop.document.ui.canvas)
                ...
                if ((box[0][1][2] / moduleCamera.video.videoWidth) < documentSizeMin) {
                    documentSize = 'small'
                    documentSizeAlert = true
                }
                if ((box[0][1][2] / moduleCamera.video.videoWidth) > documentSizeMax) {
                    documentSize = 'big'
                    documentSizeAlert = true
                }
                ...
                let videoCenterX = moduleCamera.video.videoWidth/2
                let videoCenterY = moduleCamera.video.videoHeight/2
                ...
                if (vu.sop.ui.debug.enable) {
                    ...
                    vu.sop.ui.debug.eval.push(['Doc size', Math.round((box[0][1][2] / moduleCamera.video.videoWidth)*100) + "%", color])
                ...
                if (moduleCamera.video.videoWidth < 1080 && window.innerHeight > window.innerWidth){
                    vu.sop.ui.showBottomTextAlert(vu.sop.msg.rotateScreen)
                ...
                if (isBlurry[0] == true){
                    ...
                    if(vu.sop.enableTelemetry){
                        vu.telemetry.addEvent("DocumentActivityProcess", "end", {"isBlurry" : true} );
                    }
        ...
        if (takePhoto) {
            ...
            vu.sop.ui.flash();
            picture = await moduleCamera.takePicture()
    ...
    // Continuar loopeando
    setTimeout(function () {
        if(vu.sop.document.ui.doLoop == true)
        {
...
vu.sop.document.ui.drawBox = function(predictResults) {
    let scale = moduleCamera.video.offsetHeight / moduleCamera.video.videoHeight
    try {
        ...
        let fixX = Math.round((moduleCamera.video.offsetWidth - vu.sop.document.ui.videoContainer.offsetWidth)/2)
        let fixY = Math.round((moduleCamera.video.offsetHeight - vu.sop.document.ui.videoContainer.offsetHeight)/2)
...
export default vu.sop.document.ui;
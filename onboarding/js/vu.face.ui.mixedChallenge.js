(function (root, factory) {
    if (typeof define === 'function' && define.amd) {
        ...
        root.vu.face.ui = root.vu.face.ui || {};
        root.vu.face.gestures = root.vu.face.gestures || {};
        root.vu.face.ui.gestures = factory();
    }
...
vu.face.gestures = vu.face.gestures || {};
vu.face.ui.gestures = vu.face.ui.gestures || {};
vu.sop = vu.sop || {};
vu.sop.ui = vu.sop.ui || {};
...
vu.face.ui.gestures.initialize = function(camera) {
...
// 'eyeClose', 'smile'
// Private variables - not exposed to window scope
let allChallenges = ['smile', 'lookLeft', 'lookRight', 'eyeClose'];
let numOfChallenges = 3;
let challengesConfigured = false;

// Public setter function to configure challenges (security: only accepts first configuration until release)
vu.face.ui.gestures.setChallenges = function(challenges) {
    ...
    if ((vu.face.auth && vu.face.auth.initialized) || 
        ...
        console.warn("[Security] setChallenges: Configuration already set. Call vu.face.auth.release() or vu.sop.release() first to reconfigure.");
        return false;
    }
    
    if (!Array.isArray(challenges) || challenges.length === 0) {
        console.error("[Security] setChallenges: Invalid challenges array provided.");
        return false;
    }
    
    allChallenges = challenges;
    console.log("[Security] setChallenges: Challenges configured successfully.");
    return true;
};

// Secure getter function for challenges
vu.face.ui.gestures.getChallenges = function() {
    return allChallenges;
};

// Public setter function to configure number of challenges (security: only accepts first configuration until release)
vu.face.ui.gestures.setNumOfChallenges = function(num) {
    // Check if EITHER vuFace or vuIDCard is initialized to prevent cross-module contamination
    if ((vu.face.auth && vu.face.auth.initialized) || 
        (vu.sop && vu.sop.initialized)) {
        console.warn("[Security] setNumOfChallenges: Configuration already set. Call vu.face.auth.release() or vu.sop.release() first to reconfigure.");
        return false;
    }
    
    if (!Number.isInteger(num) || num < 1 || num > 10) {
        console.error("[Security] setNumOfChallenges: Invalid number of challenges. Must be an integer between 1 and 10.");
        return false;
    }
    
    numOfChallenges = num;
    console.log("[Security] setNumOfChallenges: Number of challenges configured successfully.");
    return true;
};

// Secure getter function for number of challenges
vu.face.ui.gestures.getNumOfChallenges = function() {
    return numOfChallenges;
};

// Integrators must use vu.face.ui.gestures.setChallenges() and vu.face.ui.gestures.setNumOfChallenges()
...
vu.face.ui.gestures.elipseSvg = function(lineColor, lineWidth, backgroundColor, backgroundOpacity) {
    let width = document.getElementById('vu.face.ui.gestures.circle').offsetWidth;
    let height = document.getElementById('vu.face.ui.gestures.circle').offsetHeight;

    let relative = (height * 1148)/ 480;
	let sd110 = (relative * ((110 * 100)/1148))/100
	let sd220 = (relative * ((220 * 100)/1148))/100
	let sd66 = (relative * ((66 * 100)/1148))/100
	let sd68 = (relative * ((68 * 100)/1148))/100
...
// Removed vu.face.ui.gestures.numOfChallenges direct assignment for security - now uses private variable
...
vu.face.ui.gestures.videoResizeObserver = new ResizeObserver(entries => {
    console.trace("ResizeObserver fired in vu.face.ui.gestures.videoResizeObserver");
    try {
        console.log("vu.face.gestures.circleActiveColor, vu.face.gestures.lineWidth, vu.face.gestures.backGroundColor, vu.face.gestures.backGroundOpacity", vu.face.gestures.circleActiveColor, vu.face.gestures.lineWidth, vu.face.gestures.backGroundColor, vu.face.gestures.backGroundOpacity);
...
vu.face.ui.gestures.start = function(basePath) {
    ...
    console.log("vu.face.gestures.method", vu.face.gestures.method);
    console.log("vu.face.gestures.backgroundSize", vu.face.gestures.backgroundSize);
...
let resultsFeedbackTimeFrame = 1000; // deprecado, no aplica
let resultsValidateTimeFrame = 4000;
let resultsFeedbackPercentual = 60; // deprecado, no aplica
let resultsValidatePercentual = 30;
let resultsValidateMinTimeFrame = 2000;

// Public setter function to configure results feedback time frame (security: only accepts first configuration until release)
vu.face.ui.gestures.setResultsFeedbackTimeFrame = function(timeFrame) {
    // Check if EITHER vuFace or vuIDCard is initialized to prevent cross-module contamination
    if ((vu.face.auth && vu.face.auth.initialized) || 
        (vu.sop && vu.sop.initialized)) {
        console.warn("[Security] vu.face.ui.gestures.setResultsFeedbackTimeFrame: Configuration already set. Call vu.face.auth.release() or vu.sop.release() first to reconfigure.");
        return false;
    }
    
    if (!Number.isInteger(timeFrame) || timeFrame < 100 || timeFrame > 10000) {
        console.error("[Security] vu.face.ui.gestures.setResultsFeedbackTimeFrame: Invalid time frame. Must be an integer between 100 and 10000 milliseconds.");
        return false;
    }
    
    resultsFeedbackTimeFrame = timeFrame;
    console.log("[Security] vu.face.ui.gestures.setResultsFeedbackTimeFrame: Results feedback time frame configured successfully.");
    return true;
};

// Secure getter function for results feedback time frame
vu.face.ui.gestures.getResultsFeedbackTimeFrame = function() {
    return resultsFeedbackTimeFrame;
};

// Public setter function to configure results validate time frame (security: only accepts first configuration until release)
vu.face.ui.gestures.setResultsValidateTimeFrame = function(timeFrame) {
    // Check if EITHER vuFace or vuIDCard is initialized to prevent cross-module contamination
    if ((vu.face.auth && vu.face.auth.initialized) || 
        (vu.sop && vu.sop.initialized)) {
        console.warn("[Security] vu.face.ui.gestures.setResultsValidateTimeFrame: Configuration already set. Call vu.face.auth.release() or vu.sop.release() first to reconfigure.");
        return false;
    }
    
    if (!Number.isInteger(timeFrame) || timeFrame < 1000 || timeFrame > 30000) {
        console.error("[Security] vu.face.ui.gestures.setResultsValidateTimeFrame: Invalid time frame. Must be an integer between 1000 and 30000 milliseconds.");
        return false;
    }
    
    resultsValidateTimeFrame = timeFrame;
    console.log("[Security] vu.face.ui.gestures.setResultsValidateTimeFrame: Results validate time frame configured successfully.");
    return true;
};

// Secure getter function for results validate time frame
vu.face.ui.gestures.getResultsValidateTimeFrame = function() {
    return resultsValidateTimeFrame;
};

// Public setter function to configure results feedback percentual (security: only accepts first configuration until release)
vu.face.ui.gestures.setResultsFeedbackPercentual = function(percentage) {
    // Check if EITHER vuFace or vuIDCard is initialized to prevent cross-module contamination
    if ((vu.face.auth && vu.face.auth.initialized) || 
        (vu.sop && vu.sop.initialized)) {
        console.warn("[Security] vu.face.ui.gestures.setResultsFeedbackPercentual: Configuration already set. Call vu.face.auth.release() or vu.sop.release() first to reconfigure.");
        return false;
    }
    
    if (!Number.isInteger(percentage) || percentage < 1 || percentage > 100) {
        console.error("[Security] vu.face.ui.gestures.setResultsFeedbackPercentual: Invalid percentage. Must be an integer between 1 and 100.");
        return false;
    }
    
    resultsFeedbackPercentual = percentage;
    console.log("[Security] vu.face.ui.gestures.setResultsFeedbackPercentual: Results feedback percentual configured successfully.");
    return true;
};

// Secure getter function for results feedback percentual
vu.face.ui.gestures.getResultsFeedbackPercentual = function() {
    return resultsFeedbackPercentual;
};

// Public setter function to configure results validate percentual (security: only accepts first configuration until release)
vu.face.ui.gestures.setResultsValidatePercentual = function(percentage) {
    // Check if EITHER vuFace or vuIDCard is initialized to prevent cross-module contamination
    if ((vu.face.auth && vu.face.auth.initialized) || 
        (vu.sop && vu.sop.initialized)) {
        console.warn("[Security] vu.face.ui.gestures.setResultsValidatePercentual: Configuration already set. Call vu.face.auth.release() or vu.sop.release() first to reconfigure.");
        return false;
    }
    
    if (!Number.isInteger(percentage) || percentage < 1 || percentage > 100) {
        console.error("[Security] vu.face.ui.gestures.setResultsValidatePercentual: Invalid percentage. Must be an integer between 1 and 100.");
        return false;
    }
    
    resultsValidatePercentual = percentage;
    console.log("[Security] vu.face.ui.gestures.setResultsValidatePercentual: Results validate percentual configured successfully.");
    return true;
};

// Secure getter function for results validate percentual
vu.face.ui.gestures.getResultsValidatePercentual = function() {
    return resultsValidatePercentual;
};
...
vu.face.ui.gestures.setResultsValidateMinTimeFrame = function(timeFrame) {
    // Check if EITHER vuFace or vuIDCard is initialized to prevent cross-module contamination
    if ((vu.face.auth && vu.face.auth.initialized) || 
        (vu.sop && vu.sop.initialized)) {
        console.warn("[Security] vu.face.ui.gestures.setResultsValidateMinTimeFrame: Configuration already set. Call vu.face.auth.release() or vu.sop.release() first to reconfigure.");
        return false;
    }
    
    if (!Number.isInteger(timeFrame) || timeFrame < 500 || timeFrame > 15000) {
        console.error("[Security] vu.face.ui.gestures.setResultsValidateMinTimeFrame: Invalid time frame. Must be an integer between 500 and 15000 milliseconds.");
        return false;
    }
    
    resultsValidateMinTimeFrame = timeFrame;
    console.log("[Security] vu.face.ui.gestures.setResultsValidateMinTimeFrame: Results validate minimum time frame configured successfully.");
    return true;
};

// Secure getter function for results validate minimum time frame
vu.face.ui.gestures.getResultsValidateMinTimeFrame = function() {
    return resultsValidateMinTimeFrame;
};

let challenge;
// Private function - not exposed to window scope
function doLoop(basePath) {
    // console.log("vu.face.ui.gestures.doLoop", vu.camera.video);   
    // console.log("window.vu.face.ui.gestures.doLoop", window.vu.camera.video);               
    // if(vu.camera.video == null)
    //     return;
        
    var start = new Date();
    
    vu.face.getData().then(function (data) {
        //console.log("vu.face.getData().then", data);
        // if(vu.camera.video == null)
        //     return;

        if (data[0] == false) {
            vu.face.load(moduleCamera.video, basePath).then(
        ...
        // console.log("vu.face.ui.gestures.lastChallenge", vu.face.ui.gestures.lastChallenge);
        // console.log("vu.face.ui.gestures.challengeNum", vu.face.ui.gestures.challengeNum);
        // console.log("vu.face.ui.gestures.numOfChallenges", vu.face.ui.gestures.numOfChallenges);
        if (vu.face.ui.gestures.challengeNum == numOfChallenges) {
        ...
        /* Gesture Feedback */
        if (data[0] === true) {
            let gestureMatch = gestureMach(data);
            //console.log(data)
            ...
            vu.face.ui.gestures.results.push([gestureMatch,timeNow,imageQualityIsOK]);
            ...
            for (var i = 0; i < vu.face.ui.gestures.results.length; i++) {
                if ((timeNow - vu.face.ui.gestures.results[i][1]) < resultsValidateTimeFrame){
            ...
            if (imageQualityIsOK){
                if (gestureMatch){
                    ...
                    let timeNow = Date.now()    // Tiempo actual (milisegundos)
                    let timeFrame = 1000        // Tiempo a evaluar
                    let resultsOK = 50         // Porcentual
                    ...
                    let count = 0       // Contador de cantidad de gestos detectados en el periodo del'timeFrame'
                    let results = 0     // Contador de cantidad de gestos que machean el desafió en el periodo del 'timeFrame'
...
// Private function - not exposed to window scope
function gestureMach(data) {
    let actualGesture = data[2]
    let gestureMach = false
    let x = data[1][0]
    let y = data[1][1]
    let left;
    let right;    
    ...
    if (challenge === 'none') {
        ...
        let center = false;
        let smile = false;
        //console.debug("vu.face.gestures.permisiveNeutralChallenge", vu.face.gestures.permisiveNeutralChallenge);
...
function genChallenges() {
    ...
    if(numOfChallenges > allChallenges.length + 1) {
        numOfChallenges = allChallenges.length + 1;
    }

    let cha;
    for (i = 1; i < numOfChallenges; i++) {
        while (true) {
            cha = allChallenges[Math.floor(Math.random() * allChallenges.length)];
...
vu.face.ui.gestures.challengeStart = function() {
    let promise = new Promise(function (resolve, reject) {
        ...
        vu.face.ui.gestures.challengeReject = reject;
    });
    return promise
};
...
function challengeReset() {
    vu.face.ui.gestures.pictures = []
    vu.face.ui.gestures.challengeNum = 0
    if (vu.face.ui.gestures.picturesTags.length > 0) {
        vu.face.ui.gestures.picturesTags = [vu.face.ui.gestures.picturesTags[0]]
    };
};

// Private function - not exposed to window scope
async function challengeDoLoop() {
    let challenge = vu.face.ui.gestures.challenges[vu.face.ui.gestures.challengeNum]
    let takePhoto = false;
    let timeNow = Date.now()
    /* Evaluamos si hay suficientes resultados para sacar la foto */
    if (vu.face.ui.gestures.results.length > 2){
        if (( timeNow - vu.face.ui.gestures.results[0][1]) > resultsValidateMinTimeFrame ) {
            /* Evaluamos si se tiene que sacar la foto */
            let countTrue = 0
            for (let i = 0; i < vu.face.ui.gestures.results.length; i++) {
                 if (vu.face.ui.gestures.results[i][0] === true){
                     countTrue = countTrue + 1
                 }
            }
            //console.log("resultsValidatePercentual", resultsValidatePercentual);
            if (((countTrue*100)/vu.face.ui.gestures.results.length) > resultsValidatePercentual){
    ...
    if (takePhoto && vu.face.currentFaceSize == "ok") {     
        try {
            let taked = await moduleCamera.takePicture();
            console.log("await vu.camera.takePicture()", taked);
            vu.face.ui.gestures.pictures.push(taked);
            console.log("vu.face.ui.gestures.pictures", vu.face.ui.gestures.pictures);
            ...
            if(vu.sop.enableTelemetry){
                // Se retira el Await para que no se bloquee el proceso
                vu.telemetry.addEvent("SelfieActivityProcess" , "challengeInfo" ,
                    {
                        "challenge" : challenge,
                        "debugEvaluation" : vu.sop.ui.debug.finalEval[vu.sop.ui.debug.finalEval.length - 1]
                    });
            }
            ...
            vu.face.ui.gestures.loop = false;
            vu.sop.ui.hideBottomText();
            if (typeof vu.face.ui.gestures.challengeReject === 'function') {
                vu.face.ui.gestures.challengeReject(err);
            } else {
                console.warn('[vu.camera] No rejection callback set for gestures.');
            }
...
function drawBox(bbox) {
    scale = moduleCamera.video.offsetHeight / moduleCamera.video.videoHeight
    try {
        ...
        if (bbox[2] > moduleCamera.video.videoWidth) {
            bbox[2] = moduleCamera.video.videoWidth
        }
        if (bbox[3] > moduleCamera.video.videoHeight) {
            bbox[3] = moduleCamera.video.videoHeight
        }
        bleft = bbox[0] * scale
        btop = bbox[1] * scale
        bwidth = bbox[2] * scale
        ...
        fixX = Math.round((moduleCamera.video.offsetWidth - vu.face.ui.gestures.videoContainer.offsetWidth)/2)
        fixY = Math.round((moduleCamera.video.offsetHeight - vu.face.ui.gestures.videoContainer.offsetHeight)/2)
...
//export default vu.face.ui.gestures;

    // Return the API object for UMD
    return vu.face.ui.gestures;
}));